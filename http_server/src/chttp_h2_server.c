...
int chttp_h2_server_config_validate(const chttp_server_config *config,
  ...
  *out_protocol_config = (chttp_h2_proto_config){0};
  ...
  if (!chttp_h2_server_multiply(config->max_header_count, 2u, &header_storage_bytes) ||
      ...
      !chttp_h2_server_add(config->max_response_header_count, CHTTP_SERVER_GENERATED_HEADER_COUNT, &response_header_count) ||
      ...
      !chttp_h2_server_add(
          sizeof(":status") - 1u + 3u + sizeof("content-length") - 1u + 3u * sizeof(size_t) +
              sizeof("date") - 1u + CHTTP_SERVER_DATE_BYTES,
...
static int chttp_h2_server_stream_init(chttp_h2_server_stream *stream,
  ...
  if (!chttp_h2_server_multiply(config->max_header_count, 2u, &header_terminators) ||
    ...
    return SALTS_ERANGE;
  stream->owner = owner;
  stream->response_header_capacity = config->max_response_header_count + CHTTP_SERVER_GENERATED_HEADER_COUNT;
...
static int chttp_h2_server_submit_response_builder(chttp_h2_server_stream *stream,
  ...
  submit_status = chttp_server_response_date(builder, date);
  if (submit_status != SALTS_OK) return submit_status;
  ...
  if (date[0] != '\0')
    stream->response_headers[header_count++] =
        (chttp_h2_hpack_header){"date", sizeof("date") - 1u, date, CHTTP_SERVER_DATE_BYTES};
...
static int chttp_h2_server_websocket_status(chttp_h2_server_stream *stream,
  ...
  char status_text[4];
  char date[CHTTP_SERVER_DATE_CAPACITY];
  chttp_h2_hpack_header headers[2];
  const int date_status = chttp_server_response_date(NULL, date);
  if (date_status != SALTS_OK) return date_status;
  const int size = snprintf(status_text, sizeof(status_text), "%u", status_code);
  if (size != 3) return SALTS_EINVAL;
  headers[0] = (chttp_h2_hpack_header){":status", sizeof(":status") - 1u, status_text, (size_t)size};
  headers[1] = (chttp_h2_hpack_header){"date", sizeof("date") - 1u, date, CHTTP_SERVER_DATE_BYTES};
  stream->response_submitted = true;
  if (chttp_h2_proto_submit_response(stream->owner->protocol, stream->stream_id, headers, sizeof(headers) / sizeof(headers[0]), NULL,
                                     0u) != 0) {
    stream->response_submitted = false;
    return SALTS_ENOBUFS;
  }
...
static int chttp_h2_server_websocket_accept(chttp_h2_server_stream *stream) {
  ...
  size_t index;
  char date[CHTTP_SERVER_DATE_CAPACITY];
  const int date_status = chttp_server_response_date(builder, date);
  if (date_status != SALTS_OK) return date_status;
...
static int chttp_h2_server_websocket_dispatch(chttp_h2_server_stream *stream) {
  ...
  if (status == SALTS_EPERM) {
    ...
    status = chttp_server_request_rejection_response(&stream->request_state);
...
static int chttp_h2_server_dispatch(chttp_h2_server_stream *stream) {
  ...
  chttp_server_response_builder *builder = &stream->request_state.response_builder;
  int status;
  if (chttp_server_deadline_expired(&stream->request_state)) {
    chttp_server_request_body_close(&stream->request_state, SALTS_ETIMEDOUT);
...
static int chttp_h2_server_dispatch_or_reset(chttp_h2_server_stream *stream) {
  ...
  const uint32_t error_code =
      status == SALTS_ETIMEDOUT ? CHTTP_H2_ERR_CANCEL :
...
static int chttp_h2_server_end_headers(void *user, int32_t stream_id, int end_stream) {
  ...
  if (!stream->trailers) {
    chttp_server_deadline_start(&stream->request_state, h2->connection->server->deadlines.body_ms);
    request = chttp_h2_server_request_view(stream);
    ...
    if (status != SALTS_OK) {
      chttp_server_request_body_close(&stream->request_state, status);
      return chttp_h2_proto_submit_rst_stream(h2->protocol, stream_id,
          status == SALTS_ETIMEDOUT ? CHTTP_H2_ERR_CANCEL : CHTTP_H2_ERR_INTERNAL_ERROR) == 0
                 ? 0
                 : -1;
    }
  }
...
static int chttp_h2_server_data(void *user, int32_t stream_id, const uint8_t *data, size_t size) {
  ...
  if (stream == NULL || stream->header_block_open || stream->trailers ||
    ...
    return -1;
  if (chttp_server_deadline_expired(&stream->request_state)) {
    if (size != 0 && chttp_h2_proto_consume_connection(h2->protocol, size) != 0) return -1;
    chttp_server_request_body_close(&stream->request_state, SALTS_ETIMEDOUT);
    return chttp_h2_proto_submit_rst_stream(h2->protocol, stream_id, CHTTP_H2_ERR_CANCEL) == 0 ? 0 : -1;
  }
  ...
  if (size != 0u) {
    if (chttp_server_request_body_streaming(&stream->request_state)) {
      ...
      if (sink_status != SALTS_OK) {
        ...
        return chttp_h2_proto_submit_rst_stream(h2->protocol, stream_id,
            sink_status == SALTS_ETIMEDOUT ? CHTTP_H2_ERR_CANCEL : CHTTP_H2_ERR_INTERNAL_ERROR) == 0
                   ? 0
                   : -1;
      }
    } else {
...
int chttp_h2_server_connection_receive(chttp_h2_server_connection *h2, const void *data,
  ...
  const uint64_t generation = chttp_h2_proto_input_generation(h2->protocol);
  consumed = chttp_h2_proto_recv(h2->protocol, (const uint8_t *)data, size);
  if (!chttp_h2_proto_input_incomplete(h2->protocol)) h2->connection->request_state.deadline_ms = 0;
  else if (h2->connection->request_state.deadline_ms == 0 ||
           generation != chttp_h2_proto_input_generation(h2->protocol))
    chttp_server_deadline_start(&h2->connection->request_state, h2->connection->server->deadlines.headers_ms);
  return consumed >= 0 && (size_t)consumed == size ? SALTS_OK : SALTS_EPROTO;
}

void chttp_h2_server_connection_deadlines(chttp_h2_server_connection *h2) {
  if (h2 == NULL || h2->protocol == NULL) return;
  for (size_t index = 0; index < h2->stream_capacity; ++index) {
    chttp_h2_server_stream *stream = &h2->streams[index];
    if (!stream->active || stream->protocol_closed ||
        !chttp_server_deadline_expired(&stream->request_state)) continue;
    uint_fast64_t token = atomic_load_explicit(&stream->deferred_token, memory_order_acquire);
    if (chttp_server_deferred_token_state(token) != CHTTP_SERVER_DEFERRED_IDLE) {
      /* A worker that already claimed WRITING owns the terminal result and its storage. */
      if (chttp_server_deferred_token_state(token) == CHTTP_SERVER_DEFERRED_PENDING)
        (void)atomic_compare_exchange_strong_explicit(&stream->deferred_token, &token,
            chttp_server_deferred_token(chttp_server_deferred_token_generation(token),
                CHTTP_SERVER_DEFERRED_CANCELED), memory_order_acq_rel, memory_order_acquire);
      stream->request_state.deadline_ms = 0;
      continue;
    }
    stream->request_state.deadline_ms = 0;
    chttp_server_request_body_close(&stream->request_state, SALTS_ETIMEDOUT);
    ...
    (void)chttp_server_send_pending(h2->connection);
  }
}

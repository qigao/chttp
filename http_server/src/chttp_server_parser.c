...
static int chttp_server_parser_on_message_begin(llhttp_t *llparser) {
  ...
  if (parser->on_message_begin != NULL) parser->on_message_begin(parser->user);
  return 0;
}
...
static int chttp_server_parser_normalize_target(chttp_server_parser_impl *parser) {
  char *target = parser->target_storage;
  char *authority;
  char *suffix;
  char separator;
  ...
  size_t index;
  uri_t uri;
  if (memchr(target, '#', parser->target_size) != NULL)
    return chttp_server_parser_fail(parser, 400u);
  if (target[0] == '/' || (parser->request.method == CHTTP_METHOD_OPTIONS &&
                           parser->target_size == 1u && target[0] == '*')) return 0;
  authority = strstr(target, "://");
  if (authority == NULL) return chttp_server_parser_fail(parser, 400u);
  authority += sizeof("://") - 1u;
  suffix = authority + strcspn(authority, "/?");
  separator = *suffix;
  *suffix = '\0';
  const int parsed = uri_parse(target, &uri);
  *suffix = separator;
  if (uri.overflow_flags != 0u) return chttp_server_parser_fail(parser, 414u);
  if (parsed != 1 || uri.host[0] == '\0' ||
      (uri.component_flags & URI_COMPONENT_USERINFO) != 0u ||
      (!chttp_server_ascii_equal(uri.scheme, "http") &&
       !chttp_server_ascii_equal(uri.scheme, "https")))
    return chttp_server_parser_fail(parser, 400u);
  authority_size = (size_t)(suffix - authority);
  /* Reuse the removed scheme bytes so authority and origin-form views coexist. */
  memmove(target, authority, authority_size);
  target[authority_size] = '\0';
  if (separator != '/') *--suffix = '/';
  parser->request.target = suffix;
  parser->target_size = strlen(suffix);
  for (index = 0u; index < parser->request.header_count; ++index) {
    if (chttp_server_ascii_equal(parser->headers[index].name, "Host")) {
      parser->headers[index].value = target;
      return 0;
    }
  }
  if (parser->request.header_count >= parser->max_header_count)
    return chttp_server_parser_fail(parser, 431u);
  parser->headers[parser->request.header_count++] = (chttp_header){"Host", target};
  return 0;
}
...
static int chttp_server_parser_on_headers_complete(llhttp_t *llparser) {
  ...
  status = chttp_server_parser_normalize_target(parser);
  if (status != 0) return status;
  query = (const char *)memchr(parser->request.target, '?', parser->target_size);
  path_size = query == NULL ? parser->target_size : (size_t)(query - parser->request.target);
  if (path_size == 0u) return chttp_server_parser_fail(parser, 400u);
  memcpy(parser->path_storage, parser->request.target, path_size);
...
int chttp_server_parser_init(chttp_server_parser *parser,
  ...
  impl->on_message_begin = config->on_message_begin;

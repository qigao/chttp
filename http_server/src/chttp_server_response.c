...
int chttp_server_format_date(time_t value, char output[CHTTP_SERVER_DATE_CAPACITY]) {
  ...
  const int size = snprintf(output, CHTTP_SERVER_DATE_CAPACITY,
      "%s, %02d %s %04d %02d:%02d:%02d GMT", weekdays[utc.tm_wday], utc.tm_mday,
      months[utc.tm_mon], utc.tm_year + TM_YEAR_BASE, utc.tm_hour, utc.tm_min, utc.tm_sec);
  return size == CHTTP_SERVER_DATE_BYTES ? SALTS_OK : SALTS_ERANGE;
}

int chttp_server_response_date(const chttp_server_response_builder *builder,
                               char output[CHTTP_SERVER_DATE_CAPACITY]) {
...
static int chttp_server_response_set_header_impl(chttp_server_response *response, const char *name,
                                                 const char *value, bool allow_websocket_protocol) {
...
int chttp_server_response_append_vary(chttp_server_response *response, const char *fields) {
  ...
  for (size_t index = 0; index < builder->header_count; ++index) {
    ...
    if (strcmp(header->value, "*") == 0) return SALTS_OK;
    const size_t previous = strlen(header->value);
    if (previous > SIZE_MAX - extra - 3u) return SALTS_ERANGE;
    const size_t needed = previous + extra + 3u;
    if (needed > builder->header_storage_capacity - builder->header_storage_used ||
        extra + 2u > builder->header_storage_capacity - builder->header_wire_bytes)
      return SALTS_ENOBUFS;
    char *value = builder->header_storage + builder->header_storage_used;
    memcpy(value, header->value, previous);
    memcpy(value + previous, ", ", 2u);
    memcpy(value + previous + 2u, fields, extra + 1u);
    header->value = value;
    builder->header_storage_used += needed;
    builder->header_wire_bytes += extra + 2u;
    return SALTS_OK;
  }
...
int chttp_server_response_serialize(const chttp_server_response_builder *builder,
  ...
  if (status == SALTS_OK && date[0] != '\0') {
    line_size = snprintf(line, sizeof(line), "Date: %s\r\n", date);
    if (line_size < 0 || (size_t)line_size >= sizeof(line)) status = SALTS_EMSGSIZE;
    else status = chttp_server_output_append(output, output_capacity, inout_size, line, (size_t)line_size);
  }
...
int chttp_server_error_serialize(unsigned int status_code, unsigned char *output,
  ...
  if (output == NULL || inout_size == NULL || status_code < 400u || status_code > 599u)
    return SALTS_EINVAL;
  const int status = chttp_server_response_date(NULL, date);
  if (status != SALTS_OK) return status;
  if (status_code == 426u)
    wire_size =
        snprintf(wire, sizeof(wire),
                 ...
                 "Content-Length: %zu\r\nConnection: close\r\n\r\n%s",
                 reason, date, strlen(reason), reason);
  else
    wire_size = snprintf(wire, sizeof(wire),
                         "HTTP/1.1 %u %s\r\nDate: %s\r\nContent-Type: text/plain\r\nContent-Length: %zu\r\n"
                         "Connection: close\r\n\r\n%s",
                         status_code, reason, date, strlen(reason), reason);

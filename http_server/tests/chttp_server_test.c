...
static int chttp_static_file_test_handler(void *user, const chttp_server_request_view *request,
                                         chttp_server_response *response) {
  return chttp_server_serve_file(response, request, (const chttp_server_file_options *)user);
}

enum { CHTTP_TEST_DEADLINE_MS = 80 };

static int chttp_test_admission(void *user, const chttp_server_request_view *request,
                                chttp_server_admission_result *result) {
  size_t *calls = (size_t *)user;
  ++*calls;
  if (request->body != NULL || request->body_size != 0 || request->session != NULL)
    return SALTS_EPROTO;
  if (strcmp(request->path, "/stream-upload") == 0)
    *result = (chttp_server_admission_result){.status_code = 429, .retry_after_seconds = 7};
  else if (strcmp(request->path, "/invalid") == 0) result->status_code = 200;
  else if (strcmp(request->path, "/admit/42") == 0) {
    const char *id = chttp_server_request_param(request, "id");
    if (id == NULL || strcmp(id, "42") != 0) return SALTS_EPROTO;
  }
  return SALTS_OK;
}

static int chttp_test_authenticated_admission(void *user, const chttp_server_request_view *request,
                                              chttp_server_admission_result *result) {
  chttp_server_test_jwt_probe *probe = (chttp_server_test_jwt_probe *)user;
  (void)result;
  if (request->jwt_claims == NULL || request->jwt_claims->subject == NULL ||
      strcmp(request->jwt_claims->subject, "alice") != 0) return SALTS_EPROTO;
...
spec("CHTTP background HTTP/1.1 server") {
  it("rate limits the same peer across connections before 100 Continue and body delivery") {
    ...
    const chttp_server_route_options route = {.method = CHTTP_METHOD_POST, .path = "/stream-upload",
      .handler = chttp_server_stream_handler, .user = &probe,
      .body_open = chttp_server_stream_open, .body_close = chttp_server_stream_close};
    uint16_t port = 0;
    char response[CHTTP_SERVER_TEST_RAW_BYTES];
    size_t response_size = 0;
    check_equal(chttp_rate_limiter_init(&limiter, &limit), SALTS_OK);
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    check_equal(chttp_server_set_admission(&server, chttp_rate_limiter_admit, &limiter), SALTS_OK);
    check_equal(chttp_server_get(&server, "/ok", chttp_server_test_static, NULL), SALTS_OK);
    check_equal(chttp_server_route_with(&server, &route), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    check_equal(chttp_server_test_raw_exchange(port,
      "GET /ok HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
      response, sizeof(response), &response_size), SALTS_OK);
    check_not_null(strstr(response, "200 OK"));
    check_equal(chttp_server_test_raw_exchange(port,
      "POST /stream-upload HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\nExpect: 100-continue\r\n\r\n",
      response, sizeof(response), &response_size), SALTS_OK);
    check_not_null(strstr(response, "429"));
    check_not_null(strstr(response, "Retry-After:"));
    check_null(strstr(response, "100 Continue"));
    check_equal(chttp_server_stop(&server, CHTTP_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(probe.opens, 0u);
    check_equal(chttp_server_destroy(&server), SALTS_OK);
    chttp_rate_limiter_destroy(&limiter);
  }
  it("does not renew a header deadline when bytes trickle in") {
    chttp_server server = {0};
    chttp_server_config config = chttp_server_test_config();
    const chttp_server_deadlines deadlines = {.headers_ms = CHTTP_TEST_DEADLINE_MS * 5u};
    chttp_server_test_socket socket_value = CHTTP_SERVER_TEST_INVALID_SOCKET;
    uint16_t port = 0;
    char response[CHTTP_SERVER_TEST_RAW_BYTES];
    size_t size = 0;
    ...
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    check_equal(chttp_server_set_deadlines(&server, &deadlines), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
  ...
  it("leaves a disabled body deadline disabled after the headers complete") {
    chttp_server server = {0};
    chttp_server_config config = chttp_server_test_config();
    const chttp_server_deadlines deadlines = {.headers_ms = CHTTP_TEST_DEADLINE_MS};
    chttp_server_test_socket socket_value = CHTTP_SERVER_TEST_INVALID_SOCKET;
    uint16_t port = 0;
    char response[CHTTP_SERVER_TEST_RAW_BYTES];
    size_t size = 0;
    static const char partial[] = "POST /echo HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\nConnection: close\r\n\r\nab";
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    check_equal(chttp_server_set_deadlines(&server, &deadlines), SALTS_OK);
    ...
    check_equal(chttp_server_test_raw_receive(socket_value, response, sizeof(response), &size, true, NULL), SALTS_OK);
    check_not_null(strstr(response, "200 OK"));
    check_not_null(strstr(response, "abcd"));
    chttp_server_test_close_socket(socket_value);
    check_equal(chttp_server_stop(&server, CHTTP_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(chttp_server_destroy(&server), SALTS_OK);
  }
  it("admits before body sinks and 100 Continue with routed parameters") {
    chttp_server server = {0};
    chttp_server_config config = chttp_server_test_config();
    chttp_server_stream_probe probe = {0};
    size_t calls = 0, response_size = 0;
    uint16_t port = 0;
    char response[CHTTP_SERVER_TEST_RAW_BYTES];
    const chttp_server_route_options upload = {.method = CHTTP_METHOD_POST, .path = "/stream-upload",
        .handler = chttp_server_stream_handler, .user = &probe,
        .body_open = chttp_server_stream_open, .body_close = chttp_server_stream_close};
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    check_equal(chttp_server_set_admission(&server, chttp_test_admission, &calls), SALTS_OK);
    check_equal(chttp_server_route_with(&server, &upload), SALTS_OK);
    check_equal(chttp_server_get(&server, "/admit/:id", chttp_server_test_static, NULL), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_set_admission(&server, NULL, NULL), SALTS_EBUSY);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    check_equal(chttp_server_test_raw_exchange(port,
        "POST /stream-upload HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\nExpect: 100-continue\r\n\r\n",
        response, sizeof(response), &response_size), SALTS_OK);
    check_not_null(strstr(response, "429"));
    check_not_null(strstr(response, "Retry-After: 7"));
    check_null(strstr(response, "100 Continue"));
    check_equal(chttp_server_test_raw_exchange(port,
        "GET /admit/42 HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
        response, sizeof(response), &response_size), SALTS_OK);
    check_not_null(strstr(response, "200 OK"));
    check_equal(chttp_server_test_raw_exchange(port,
        "GET /invalid HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
        response, sizeof(response), &response_size), SALTS_OK);
    check_not_null(strstr(response, "500"));
    check_equal(chttp_server_stop(&server, CHTTP_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(calls, 3u);
    check_equal(probe.opens, 0u);
    check_equal(probe.closes, 0u);
    check_equal(chttp_server_destroy(&server), SALTS_OK);
  }

  it("expires H1 headers bodies and deferred work without accepting late replies") {
    chttp_server server = {0};
    chttp_server_config config = chttp_server_test_config();
    const chttp_server_deadlines deadlines = {CHTTP_TEST_DEADLINE_MS, CHTTP_TEST_DEADLINE_MS,
                                              CHTTP_TEST_DEADLINE_MS};
    chttp_server_stream_probe body = {0};
    chttp_server_test_deferred_probe deferred = {0};
    atomic_init(&deferred.acquired, 0);
    const chttp_server_route_options upload = {.method = CHTTP_METHOD_POST, .path = "/stream-upload",
        .handler = chttp_server_stream_handler, .user = &body,
        .body_open = chttp_server_stream_open, .body_close = chttp_server_stream_close};
    const char *requests[] = {
        "GET /ok HTTP/1.1\r\nHost: local",
        "POST /stream-upload HTTP/1.1\r\nHost: localhost\r\nContent-Length: 4\r\n\r\nab",
    ...
    check_equal(chttp_server_set_deadlines(&server, &deadlines), SALTS_OK);
    check_equal(chttp_server_route_with(&server, &upload), SALTS_OK);
    check_equal(chttp_server_get(&server, "/deferred", chttp_server_test_deferred, &deferred), SALTS_OK);
    check_equal(chttp_server_get(&server, "/slow", chttp_test_slow_handler, NULL), SALTS_OK);
    check_equal(chttp_server_get(&server, "/ok", chttp_server_test_static, NULL), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_set_deadlines(&server, &deadlines), SALTS_EBUSY);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    for (size_t index = 0; index < sizeof(requests) / sizeof(requests[0]); ++index) {
      check_equal(chttp_server_test_raw_exchange(port, requests[index], response, sizeof(response), &size), SALTS_OK);
      check_equal(size, 0u);
    }
    check_equal(atomic_load_explicit(&deferred.acquired, memory_order_acquire), 1);
    const chttp_server_deferred_response late = {.size = sizeof(late), .status_code = 204};
    check_equal(chttp_server_deferred_reply(&deferred.handle, &late), SALTS_ENOENT);
    check_equal(chttp_server_test_raw_exchange(port,
        "GET /ok HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n",
        response, sizeof(response), &size), SALTS_OK);
    check_not_null(strstr(response, "200 OK"));
    check_equal(chttp_server_stop(&server, CHTTP_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(body.opens, 1u);
    check_equal(body.closes, 1u);
    check_equal(body.close_status, SALTS_ETIMEDOUT);
  ...
  it("serves conditional and ranged files with reusable HTTP/1.1 connections") {
    ...
    static const struct {
      const char *name, *value, *second_name, *second_value, *body, *range;
      unsigned int status;
    } cases[] = {
      {"Range", "bytes=2-5", NULL, NULL, "2345", "bytes 2-5/10", 206},
      {"Range", "bytes=7-", NULL, NULL, "789", "bytes 7-9/10", 206},
      {"Range", "bytes=-3", NULL, NULL, "789", "bytes 7-9/10", 206},
      {"Range", "bytes=8-99", NULL, NULL, "89", "bytes 8-9/10", 206},
      {"Range", "bytes=10-", NULL, NULL, "", "bytes */10", 416},
      {"Range", "bytes=-0", NULL, NULL, "", "bytes */10", 416},
      {"Range", "bytes=0-1,4-5", NULL, NULL, payload, NULL, 200},
      {"Range", "bytes=18446744073709551616-", NULL, NULL, payload, NULL, 200},
      {"Range", "bytes=5-2", NULL, NULL, payload, NULL, 200},
      {"If-None-Match", "W/\"revision,one\"", NULL, NULL, "", NULL, 304},
      {"If-None-Match", "\"other\", \"revision,one\"", "Range", "bytes=0-1", "", NULL, 304},
      {"If-Match", "W/\"revision,one\"", NULL, NULL, "", NULL, 412},
      {"If-Match", "*", NULL, NULL, payload, NULL, 200},
      {"If-Match", "\"other\"", "if-match", "\"revision,one\"", payload, NULL, 200},
      {"If-None-Match", "\"other\"", "if-none-match", "\"revision,one\"", "", NULL, 304},
      {"Range", "bytes=0-1", "If-Range", "\"revision,one\"", "01", "bytes 0-1/10", 206},
      {"Range", "bytes=0-1", "If-Range", "W/\"revision,one\"", payload, NULL, 200},
      {"If-None-Match", "\"other\"", "If-Modified-Since", "Sun, 06 Nov 2094 08:49:37 GMT", payload, NULL, 200},
      {"If-Unmodified-Since", "Sun, 06 Nov 1994 08:49:37 GMT", NULL, NULL, "", NULL, 412},
      {"If-Unmodified-Since", "Sunday, 06-Nov-94 08:49:37 GMT", NULL, NULL, "", NULL, 412},
      {"If-Unmodified-Since", "Sun Nov  6 08:49:37 1994", NULL, NULL, "", NULL, 412},
      {"If-Unmodified-Since", "Sun, 31 Feb 1994 08:49:37 GMT", NULL, NULL, payload, NULL, 200},
      {"Range", "bytes=0-1", "If-Range", " \"revision,one\" \t", "01", "bytes 0-1/10", 206},
      {"If-Unmodified-Since", "Sun, 06 Nov 1994 08:49:37 GMT", "If-Unmodified-Since",
       "Sun, 06 Nov 1994 08:49:37 GMT", payload, NULL, 200},
      {"If-Modified-Since", "Sun, 06 Nov 2094 08:49:37 GMT", "If-Modified-Since",
       "Sun, 06 Nov 2094 08:49:37 GMT", payload, NULL, 200},
      {"If-Modified-Since", "not-a-date", NULL, NULL, payload, NULL, 200}
    };
    char *path = tt_make_temp_file("chttp-static", ".txt");
    char *empty_path = tt_make_temp_file("chttp-static-empty", ".bin");
    chttp_server_file_options file = {.path = path, .etag = "\"revision,one\""};
    chttp_server_file_options empty_file = {.path = empty_path};
    chttp_server server = {0};
    chttp_client client = {0};
    chttp_server_config config = chttp_server_test_config();
    chttp_client_config client_config = chttp_server_test_client_config();
    char uri[64], modified[64];
    uint16_t port = 0;
    chttp_response response = {0};
    check_not_null(path);
    check_not_null(empty_path);
    check_equal(tt_write_file(path, payload, sizeof(payload) - 1), 0);
    config.stream_chunk_bytes = 2;
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    check_equal(chttp_server_get(&server, "/static", chttp_static_file_test_handler, &file), SALTS_OK);
    check_equal(chttp_server_get(&server, "/empty", chttp_static_file_test_handler, &empty_file), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    check_greater(snprintf(uri, sizeof(uri), "tcp://127.0.0.1:%u", port), 0);
    check_equal(chttp_client_init(&client, &client_config), SALTS_OK);
    check_equal(chttp_server_test_call(&client, uri, "/static", NULL, 0, &response), SALTS_OK);
    check_equal(response.status_code, 200u);
    check_equal(chttp_response_header(&response, "Content-Type"), "text/plain; charset=utf-8");
    check_equal(chttp_response_header(&response, "ETag"), file.etag);
    check_not_null(chttp_response_header(&response, "Last-Modified"));
    (void)snprintf(modified, sizeof(modified), "%s", chttp_response_header(&response, "Last-Modified"));
    chttp_response_destroy(&response);
    for (size_t index = 0; index < sizeof(cases) / sizeof(cases[0]); ++index) {
      const chttp_header headers[] = {{cases[index].name, cases[index].value},
                                     {cases[index].second_name, cases[index].second_value}};
      check_equal(chttp_server_test_call(&client, uri, "/static", headers,
          cases[index].second_name != NULL ? 2 : 1, &response), SALTS_OK);
      check_equal(response.status_code, cases[index].status);
      check_equal(response.body_size, strlen(cases[index].body));
      if (response.body_size != 0) check_equal(response.body, cases[index].body, response.body_size);
      if (cases[index].range != NULL)
        check_equal(chttp_response_header(&response, "Content-Range"), cases[index].range);
      chttp_response_destroy(&response);
    }
    const chttp_header conditional = {"If-Modified-Since", modified};
    check_equal(chttp_server_test_call(&client, uri, "/static", &conditional, 1, &response), SALTS_OK);
    check_equal(response.status_code, 304u);
    chttp_response_destroy(&response);
    check_equal(chttp_server_test_method_call(&client, uri, "/static", CHTTP_METHOD_HEAD, &response), SALTS_OK);
    check_equal(response.status_code, 200u);
    check_equal(response.body_size, 0u);
    check_equal(chttp_response_header(&response, "Content-Length"), "10");
    chttp_response_destroy(&response);
    check_equal(chttp_server_test_call(&client, uri, "/empty", NULL, 0, &response), SALTS_OK);
    check_equal(response.status_code, 200u);
    check_equal(response.body_size, 0u);
    chttp_response_destroy(&response);
    const chttp_header empty_range = {"Range", "bytes=0-"};
    check_equal(chttp_server_test_call(&client, uri, "/empty", &empty_range, 1, &response), SALTS_OK);
    check_equal(response.status_code, 416u);
    check_equal(chttp_response_header(&response, "Content-Range"), "bytes */0");
    chttp_response_destroy(&response);
    check_equal(chttp_client_destroy(&client, CHTTP_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(chttp_server_stop(&server, CHTTP_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(chttp_server_destroy(&server), SALTS_OK);
    check_equal(tt_remove_file(path), 0);
    check_equal(tt_remove_file(empty_path), 0);
    free(path);
    free(empty_path);
  }
  ...
  it("validates an HS256 Bearer token before invoking the route") {
    ...
    check_equal(chttp_server_set_admission(&server, chttp_test_authenticated_admission, &probe), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    ...
    check_equal(probe.admission_calls, 1u);

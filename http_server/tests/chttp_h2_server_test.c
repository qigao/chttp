...
static int chttp_h2_static_file_handler(void *user, const chttp_server_request_view *request,
                                      chttp_server_response *response) {
...
enum { CHTTP_H2_TEST_DEADLINE_MS = 80 };

static int chttp_h2_test_admission(void *user, const chttp_server_request_view *request,
...
spec("CHTTP background HTTP/2 server") {
  it("shares rate quota across H2 streams and rejects DATA before opening the body sink") {
    ...
    const chttp_h2_hpack_header upload[] = {
      {":method", 7, "POST", 4}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/echo", 5}};
    chttp_rate_limiter limiter = {0};
    const chttp_rate_limit_config limit = {CHTTP_RATE_LIMIT_GLOBAL, 1, 1, 1, UINT32_MAX, 0};
    chttp_server server = {0};
    chttp_server_config config = chttp_h2_server_test_config();
    chttp_h2_server_stream_probe body = {0};
    const chttp_server_route_options route = {.method = CHTTP_METHOD_POST, .path = "/echo",
      .handler = chttp_h2_server_stream_handler, .user = &body,
      .body_open = chttp_h2_server_stream_open, .body_close = chttp_h2_server_stream_close};
    ...
    check_equal(chttp_server_get(&server, "/ok", chttp_h2_server_test_version_handler, NULL), SALTS_OK);
    ...
    check_equal(chttp_h2_server_test_peer_submit(&peer, normal, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit_body(&peer, upload, 4, "data", 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit(&peer, normal, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_pump(&peer, socket_value, 3), SALTS_OK);
    check_equal(peer.results[0].status, 200u);
    check_equal(peer.results[1].status, 429u);
    check_equal(peer.results[2].status, 429u);
    chttp_h2_server_test_peer_destroy(&peer);
    chttp_h2_server_test_socket_close(socket_value);
    check_equal(chttp_server_stop(&server, CHTTP_H2_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(body.opens, 0u);
    check_equal(chttp_server_destroy(&server), SALTS_OK);
    chttp_rate_limiter_destroy(&limiter);
  }
  it("rejects admission before opening a H2 body sink and preserves adjacent streams") {
    const chttp_h2_hpack_header blocked[] = {
      {":method", 7, "POST", 4}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/echo", 5}};
    const chttp_h2_hpack_header normal[] = {
      {":method", 7, "GET", 3}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/ok", 3}};
    const chttp_h2_hpack_header slow[] = {
      {":method", 7, "GET", 3}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/slow-admission", 15}};
    chttp_server server = {0};
    chttp_server_config config = chttp_h2_server_test_config();
    chttp_h2_server_stream_probe body = {0};
    size_t calls = 0;
    ...
    uint16_t port = 0;
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    check_equal(chttp_server_set_admission(&server, chttp_h2_test_admission, &calls), SALTS_OK);
    const chttp_server_deadlines deadlines = {.body_ms = CHTTP_H2_TEST_DEADLINE_MS};
    ...
    check_equal(chttp_h2_server_test_socket_connect(port, &socket_value), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_init(&peer), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit_body(&peer, blocked, 4, "data", 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit(&peer, normal, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_pump(&peer, socket_value, 2), SALTS_OK);
    check_equal(peer.results[0].status, 429u);
    check_equal(peer.results[1].status, 200u);
    check_equal(chttp_h2_server_test_peer_submit(&peer, slow, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_pump(&peer, socket_value, 3), SALTS_OK);
    check_equal(peer.results[2].error_code, CHTTP_H2_ERR_CANCEL);
    chttp_h2_server_test_peer_destroy(&peer);
    chttp_h2_server_test_socket_close(socket_value);
    check_equal(chttp_server_stop(&server, CHTTP_H2_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(body.opens, 0u);
  ...
  it("expires H2 body and deferred streams while keeping their connection usable") {
    const chttp_h2_hpack_header upload[] = {
      {":method", 7, "POST", 4}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/echo", 5}};
    const chttp_h2_hpack_header waiting[] = {
      {":method", 7, "GET", 3}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/deferred", 9}};
    const chttp_h2_hpack_header normal[] = {
      {":method", 7, "GET", 3}, {":scheme", 7, "http", 4},
      {":authority", 10, "localhost", 9}, {":path", 5, "/ok", 3}};
    chttp_server server = {0};
    ...
    const chttp_server_deadlines deadlines = {CHTTP_H2_TEST_DEADLINE_MS, CHTTP_H2_TEST_DEADLINE_MS,
                                              CHTTP_H2_TEST_DEADLINE_MS};
    chttp_h2_server_stream_probe body = {0};
    ...
    check_equal(chttp_server_set_deadlines(&server, &deadlines), SALTS_OK);
    check_equal(chttp_server_route_with(&server, &route), SALTS_OK);
    check_equal(chttp_server_get(&server, "/deferred", chttp_h2_server_test_deferred_handler, &deferred), SALTS_OK);
    check_equal(chttp_server_get(&server, "/ok", chttp_h2_server_test_version_handler, NULL), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    check_equal(chttp_h2_server_test_socket_connect(port, &socket_value), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_init(&peer), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit_tunnel(&peer, upload, 4, &stream_id), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit(&peer, waiting, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_submit(&peer, normal, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_pump(&peer, socket_value, 3), SALTS_OK);
    check_equal(peer.results[0].error_code, CHTTP_H2_ERR_CANCEL);
    check_equal(peer.results[1].error_code, CHTTP_H2_ERR_CANCEL);
    ...
    check_equal(chttp_h2_server_test_peer_submit(&peer, normal, 4), SALTS_OK);
    check_equal(chttp_h2_server_test_peer_pump(&peer, socket_value, 4), SALTS_OK);
    check_equal(peer.results[3].status, 200u);
    chttp_h2_server_test_peer_destroy(&peer);
    chttp_h2_server_test_socket_close(socket_value);
    check_equal(chttp_server_stop(&server, CHTTP_H2_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
  ...
  it("times out a preface followed by a partial first frame in the same receive") {
    const char input[] = "PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n\x00";
    chttp_server server = {0};
    chttp_server_config config = chttp_h2_server_test_config();
    const chttp_server_deadlines deadlines = {.headers_ms = CHTTP_H2_TEST_DEADLINE_MS};
    chttp_h2_server_test_socket socket_value = CHTTP_H2_SERVER_TEST_INVALID_SOCKET;
    uint16_t port = 0;
    char buffer[256];
    check_equal(chttp_server_init(&server, &config), SALTS_OK);
    ...
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    check_equal(chttp_h2_server_test_socket_connect(port, &socket_value), SALTS_OK);
    check_equal(chttp_h2_server_test_socket_send(socket_value, input, sizeof(input) - 1u), SALTS_OK);
    int received;
    do { received = recv(socket_value, buffer, sizeof(buffer), 0); } while (received > 0);
    check_equal(received, 0);
    chttp_h2_server_test_socket_close(socket_value);
    check_equal(chttp_server_stop(&server, CHTTP_H2_SERVER_TEST_TIMEOUT_MS), SALTS_OK);
    check_equal(chttp_server_destroy(&server), SALTS_OK);
  }
  ...
  it("serves an h2c prior-knowledge request through middleware and routing") {
    ...
    check_not_null(chttp_response_header(&response, "date"));
  ...
  it("uploads and downloads files over one HTTP/2 connection") {
    ...
    check_equal(chttp_server_get(&server, "/file", chttp_h2_server_test_file_handler, upload_path),
                SALTS_OK);
    check_equal(chttp_server_get(&server, "/static", chttp_h2_static_file_handler, upload_path), SALTS_OK);
    check_equal(chttp_server_start(&server), SALTS_OK);
    check_equal(chttp_server_port(&server, &port), SALTS_OK);
    check_equal(chttp_h2_server_test_endpoint(port, uri, sizeof(uri), authority, sizeof(authority)),
                SALTS_OK);
    ...
    chttp_header range = {"Range", "bytes=3-6"};
    options.headers = &range;
    options.header_count = 1;
    check_equal(chttp_get(&client, &options, &response, &error), SALTS_OK);
    check_equal(response.status_code, 206u);
    check_equal(response.body_size, 4u);
    check_equal(response.body, payload + 3, 4u);
    check_equal(chttp_response_header(&response, "Content-Range"), "bytes 3-6/15");
    char etag[80];
    check_not_null(chttp_response_header(&response, "ETag"));
    (void)snprintf(etag, sizeof(etag), "%s", chttp_response_header(&response, "ETag"));
    check_equal(strncmp(etag, "W/", 2), 0);
    chttp_response_destroy(&response);
    const chttp_header tag_list[] = {{"If-None-Match", "\"other\""}, {"If-None-Match", etag}};
    options.headers = tag_list;
    options.header_count = sizeof(tag_list) / sizeof(tag_list[0]);
    check_equal(chttp_get(&client, &options, &response, &error), SALTS_OK);
    check_equal(response.status_code, 304u);
    chttp_response_destroy(&response);
    options.headers = &range;
    options.header_count = 1;
    range = (chttp_header){"If-None-Match", etag};
    check_equal(chttp_get(&client, &options, &response, &error), SALTS_OK);
    check_equal(response.status_code, 304u);
    check_equal(response.body_size, 0u);
    chttp_response_destroy(&response);
    range = (chttp_header){"Range", "bytes=100-"};
    check_equal(chttp_get(&client, &options, &response, &error), SALTS_OK);

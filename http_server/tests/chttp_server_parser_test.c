...
spec("CHTTP server request parser") {
  it("generates Date for successful and built-in error responses without replacing application Date") {
    chttp_server_config config = {.max_response_header_count = 1,
      ...
      .max_buffered_response_body_bytes = 128};
    chttp_server_response_builder builder = {0};
    chttp_server_response response = {&builder};
    chttp_server_request_view request = {.http_major = 1, .http_minor = 1,
      .method = CHTTP_METHOD_GET};
    unsigned char wire[512] = {0};
    size_t size = 0;
    check_equal(chttp_server_response_builder_init(&builder, &config), SALTS_OK);
    check_equal(chttp_server_response_set_header(&response, "X-Test", "full"), SALTS_OK);
    check_equal(chttp_server_reply(&response, 204, NULL, NULL, 0), SALTS_OK);
    check_equal(chttp_server_response_serialize(&builder, &request, wire, sizeof(wire) - 1, &size), SALTS_OK);
    wire[size] = 0;
    check_not_null(strstr((char *)wire, "\r\nDate: "));
    check_not_null(strstr((char *)wire, " GMT\r\n"));
    chttp_server_response_builder_reset(&builder);
    check_equal(chttp_server_response_set_header(&response, "date", "Sun, 06 Nov 1994 08:49:37 GMT"), SALTS_OK);
    check_equal(chttp_server_reply(&response, 204, NULL, NULL, 0), SALTS_OK);
    size = 0;
    check_equal(chttp_server_response_serialize(&builder, &request, wire, sizeof(wire) - 1, &size), SALTS_OK);
    wire[size] = 0;
    check_not_null(strstr((char *)wire, "\r\ndate: Sun, 06 Nov 1994 08:49:37 GMT\r\n"));
    check_null(strstr((char *)wire, "\r\nDate:"));
    size = 0;
  ...
  it("normalizes absolute targets and uses their authority instead of Host") {
    static const struct { const char *wire; const char *target; const char *host; } cases[] = {
      {"GET http://example.com/a?q=1 HTTP/1.1\r\nHost: wrong.test\r\n\r\n", "/a?q=1", "example.com"},
      {"GET https://[::1]:8443 HTTP/1.1\r\nHost: wrong.test\r\n\r\n", "/", "[::1]:8443"},
    ...
    for (size_t index = 0; index < sizeof(cases) / sizeof(cases[0]); ++index) {
      ...
      check_equal(chttp_server_parser_test_execute_fragments(&parser, cases[index].wire,
                    strlen(cases[index].wire), 1, &http_status), SALTS_OK);
      check_equal(probe.requests, 1);
      check_equal(probe.target, cases[index].target);
      check_equal(probe.host, cases[index].host);
      check_equal(probe.path[0], '/');
      chttp_server_parser_destroy(&parser);
    }
  }

  it("rejects absolute targets with userinfo, unsupported schemes and invalid authority") {
    static const char *const cases[] = {
      "GET http://user@example.com/a HTTP/1.1\r\nHost: x\r\n\r\n",
    ...
    for (size_t index = 0; index < sizeof(cases) / sizeof(cases[0]); ++index) {
      ...
      unsigned int http_status = 0;
      check_equal(chttp_server_parser_init(&parser, &config), SALTS_OK);
      check_equal(chttp_server_parser_test_execute(&parser, cases[index], &http_status), SALTS_EPROTO);
  ...
  it("resets normalized authority and target storage before a pipelined origin request") {
    ...
    check_equal(probe.path, "/next");
    check_equal(probe.host, "next.test");
    chttp_server_parser_destroy(&parser);
  }

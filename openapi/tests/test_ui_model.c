...
int main(void) {
    ...
    const oa_ui_operation *operations =
        (const oa_ui_operation *)view->operations.data;
    /* Preserve path insertion order, but normalize methods to the browser order. */
    ...
    /* GET must precede POST even though POST was declared first in source. */
    ...
    /* Missing optional text is represented as an empty view. */

...
HTTP 客户端通过 `CHttp::Client` 与 `<http_client/http.h>` 使用；服务端通过
`CHttp::Server` 与 `<http_server/http.h>` 使用。两端分别编入 RPC。
库名和头文件布局已调整，调用方需要重新编译。llhttp 为私有实现依赖。
...
compression、proxy、multipart 表单/多段 Range、deferred streaming body 或自动 retry。
...
WebSocket endpoint。
H1 request target 接受无 fragment 的 origin-form、HTTP/HTTPS absolute-form 和 `OPTIONS *`。
absolute-form 在进入 route、JWT 和 middleware 前转为 origin-form，并以 URI authority 替换
请求视图中的 Host；HTTP/1.1 仍要求原报文恰有一个非空 Host。拒绝 userinfo、未知 scheme、
无效 authority；URI authority 超出 Salts URI parser 的固定容量返回 414。路径与 query 仍受
`max_target_bytes` 约束，不受 URI parser 的 path/query 数组容量限制。
规范化复用 parser 的 target/path 存储，视图在回调返回或 parser reset 后失效。
Transfer-Encoding 仅接受 HTTP/1.1 的精确
...
H1/H2 响应在应用未设置 Date 时自动生成 IMF-fixdate（英文、UTC），涵盖普通响应、deferred
响应、H2 WebSocket 开启响应与内置错误响应。应用提供的 Date 按大小写不敏感匹配并保留，
不会重复添加。自动字段使用固定临时存储，不消耗应用响应头配额；H2 的生成字段数组与
HPACK 输出预算包含 Date。取时/格式化失败沿原响应错误路径返回，不发送半成品。
依据：[RFC 9112 §3.2.2](https://www.rfc-editor.org/rfc/rfc9112.html#section-3.2.2)、
...
文件服务在 HTTP Server 层处理缓存条件、单段 Range 和 MIME；公共文件运行时仅增加有界区间读取，
不接触 HTTP 字段。每次响应由 server owner 持有 transfer，最多一个异步 read-at 在途，
按顺序交付固定大小 chunk；关闭、取消、失败均复用已有 drain/cleanup。区间 EOF 是选定长度
耗尽，原整文件接口继续检查物理 EOF。文件必须由应用保证在传输期间不被原地修改；
路径绑定也必须在响应期间保持稳定；推荐发布到不同版本路径，由路由选择版本。
元数据 ETag 为弱验证器，强 ETag 由应用提供并保证版本唯一性。
新入口不改变原 `chttp_server_response_file()` 的状态码或缓存语义。测试覆盖 H1/H2、条件优先级、
空文件、区间边界、HEAD 与 keep-alive；回滚可由应用恢复调用原文件响应入口。

使用 `chttp_server_serve_file(response, request, &options)`，其中 `options.path` 为应用选定的
本地文件，`content_type` 可指定 MIME（NULL 按扩展名选择），`etag` 可提供带双引号的版本标签。
该入口不将 URL 直接映射到磁盘路径，不进行目录遍历。不存在文件等文件系统错误返回给 handler。

| 请求 | 行为 |
| --- | --- |
| 普通 GET / HEAD | 200、ETag、Last-Modified、Accept-Ranges；HEAD 不发送正文 |
| If-Match / If-Unmodified-Since | 优先检查前置条件，不满足返回 412 |
| If-None-Match / If-Modified-Since | 匹配返回 304；存在 ETag 条件时忽略对应日期条件 |
| 单段 bytes Range | 支持首尾、开放末尾和 suffix，返回 206 / Content-Range |
| 合法但不可满足的 Range | 416 / `Content-Range: bytes */size` |
| If-Range | 只接受匹配的应用强 ETag；弱标签、日期或不匹配时返回完整表示 |
| 无效/未知单位/多段 Range | 忽略 Range，返回完整表示 |

重复 ETag 条件头按列表合并；重复或无效日期条件忽略。日期解析支持 IMF-fixdate、RFC 850 和
asctime 格式，复用 `Salts::DateTimeParser` 并在 HTTP 层转换旧格式。
普通头字段仍受原有配额约束。更多规范见
[RFC 9110 条件顺序](https://www.rfc-editor.org/rfc/rfc9110.html#section-13.2.2)与
[Range](https://www.rfc-editor.org/rfc/rfc9110.html#section-14.2)。

可选的 `chttp_server_set_admission()` 在 JWT 之后、正文之前执行拒绝策略；
...
内置 CORS 可通过 `chttp_server_use_cors()` 注册，支持来源白名单、凭据、预检、Expose-Headers
与 Vary；配置、生命周期和 JWT 执行边界见 [CORS 中间件](../http_server/README.md#cors-中间件)。
...
`http_server/tests/chttp_jwt_test.c`、`http_server/tests/chttp_server_test.c`、`http_server/tests/chttp_h2_server_test.c` 与
`tests/integration/chttp_websocket_test.c`。

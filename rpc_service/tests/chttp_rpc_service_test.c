...
static const cmeta_type_identity ADD_REQUEST_ID =
    CMETA_TYPE_ID_ATOM_INIT("test.crpc.AddRequest");
static const cmeta_type_identity ADD_RESPONSE_ID =
    CMETA_TYPE_ID_ATOM_INIT("test.crpc.AddResponse");
...
static const cmeta_data_field_desc ADD_REQUEST_FIELDS[] = {
    {"test.crpc.AddRequest.left", "left", offsetof(AddRequest, left),
     &salts_uint32_cmeta_data},
    {"test.crpc.AddRequest.right", "right", offsetof(AddRequest, right),
     &salts_uint32_cmeta_data},
    {"test.crpc.AddRequest.scale", "scale", offsetof(AddRequest, scale),
     &salts_uint32_cmeta_data}};
...
static const cmeta_data_desc ADD_REQUEST_DATA = {
    ...
    .stable_id = "test.crpc.AddRequest.data",
...
static const cmeta_data_field_desc ADD_RESPONSE_FIELDS[] = {
    {"test.crpc.AddResponse.sum", "sum", offsetof(AddResponse, sum),
     &salts_uint32_cmeta_data}};
...
static const cmeta_data_desc ADD_RESPONSE_DATA = {
    .struct_size = sizeof(cmeta_data_desc),
    .abi_version = CMETA_DATA_DESC_ABI_VERSION,
    .stable_id = "test.crpc.AddResponse.data",
...
static DataBindStatus chttp_rpc_service_test_execute(
  ...
  if (request.left == UINT32_C(999))
    return data_bind_binding_plan_write_outcome(
        plan, provider, &frame, 7, outcome, diagnostic);
...
static crpc_server_config chttp_rpc_service_test_server_config(void) {
  const chttp_server_config http = {
      .host = "127.0.0.1",
      .port = 0u,
      .backlog = 8u,
      .network = chttp_rpc_service_test_network(8u),
      .route_capacity = 4u,
...
static crpc_client_config chttp_rpc_service_test_client_config(void) {
  const chttp_client_config http = {
      .network = chttp_rpc_service_test_network(4u),
      .request_capacity = 4u,
...
static cserde_status chttp_rpc_service_encode_params(
  ...
  const chttp_rpc_service_params *params =
      (const chttp_rpc_service_params *)user;
  cserde_status status;
  if (params == NULL || writer == NULL) return CSERDE_INVALID_ARGUMENT;
  ...
  if (status == CSERDE_OK) {
    if (params->bad_left) {
      ...
      status = chttp_rpc_service_write(
          ...
          (cserde_token){
              .kind = CSERDE_STRING,
              .value.slice = {bad, 3u, CSERDE_VIEW_STABLE}});
    } else {
      status = chttp_rpc_service_write(
          writer,
          (cserde_token){.kind = CSERDE_UINT,
                         .value.uint = params->left});
    }
  }
  if (status == CSERDE_OK && params->count >= 2u)
    status = chttp_rpc_service_write(
        writer,
        (cserde_token){.kind = CSERDE_UINT,
                       .value.uint = params->right});
  if (status == CSERDE_OK && params->count >= 3u)
    status = chttp_rpc_service_write(
        writer,
        (cserde_token){.kind = CSERDE_UINT,
                       .value.uint = params->scale});
  if (status == CSERDE_OK)
    status = chttp_rpc_service_write(
...
static int chttp_rpc_service_call(
    crpc_client *client, const char *uri, uint64_t request_id,
    chttp_rpc_service_params *params,
    crpc_response *response, crpc_error *error) {
  const crpc_method method = {.name = "calc.add"};
  const crpc_options options = {
      .connection_uri = uri,
      .authority = "localhost",
      .target = "/rpc",
      .method = method,
      ...
      .params_user = params};
  return crpc_request_reply(client, &options, response, error);
}

spec("CHttp::RpcService generated RPC MethodPlan") {
  it("executes success defaults invalid params and native status distinctly") {
    ...
    crpc_error error = {0};
    chttp_rpc_service_params params = {0};
    ...
    params = (chttp_rpc_service_params){
        .left = 3u, .right = 4u, .scale = 2u, .count = 3u};
    check_equal(
        chttp_rpc_service_call(
            &client, uri, 1u, &params, &response, &error),
        SALTS_OK);
    ...
    params = (chttp_rpc_service_params){
        .left = 3u, .right = 4u, .count = 2u};
    check_equal(
        chttp_rpc_service_call(
            &client, uri, 2u, &params, &response, &error),
        SALTS_OK);
    check_equal(response.kind, CRPC_RESPONSE_RESULT);
    check_equal(cserde_reader_next(response.value.result, &token), CSERDE_OK);
    check_equal(token.kind, CSERDE_UINT);
    ...
    response = (crpc_response){0};
    params = (chttp_rpc_service_params){
        .right = 4u, .count = 2u, .bad_left = 1};
    check_equal(
        chttp_rpc_service_call(
            &client, uri, 3u, &params, &response, &error),
        SALTS_OK);
    check_equal(response.kind, CRPC_RESPONSE_REMOTE_ERROR);
    check_equal(
        response.value.remote_error.code,
        (int64_t)-32602);
    check_equal(
        response.value.remote_error.message.data,
        (const unsigned char *)"Invalid params", 14u);
    crpc_response_destroy(&response);

    response = (crpc_response){0};
    params = (chttp_rpc_service_params){
        .left = 999u, .right = 1u, .count = 2u};
    check_equal(
        chttp_rpc_service_call(
            &client, uri, 4u, &params, &response, &error),
        SALTS_OK);
    check_equal(response.kind, CRPC_RESPONSE_REMOTE_ERROR);
    check_equal(
        response.value.remote_error.code,
    ...
    check_equal(
        response.value.remote_error.message.data,
        (const unsigned char *)"Native status", 13u);
    check_not_null(response.value.remote_error.data);
    check_equal(
        cserde_reader_next(
            response.value.remote_error.data, &token),
        CSERDE_OK);
    ...
    check_equal(
        crpc_client_destroy(
            &client, CHTTP_RPC_SERVICE_TEST_TIMEOUT_MS),
        SALTS_OK);
    check_equal(
        crpc_server_stop(
            &server, CHTTP_RPC_SERVICE_TEST_TIMEOUT_MS),
        SALTS_OK);

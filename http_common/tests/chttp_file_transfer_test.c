...
spec("CHTTP asynchronous file transfer") {
  it("admits reads from two files sharing one runtime") {
    static const char payload[] = "shared runtime";
    char *path = tt_make_temp_file("chttp-shared-read", ".bin");
    cflow_io_file_runtime runtime = {0};
    cflow_io_file_runtime_config config = {.backend_kind = chttp_file_test_backend(),
      .file_capacity = 2u, .request_capacity = 2u, .command_capacity = 4u,
      .completion_batch_capacity = 2u};
    chttp_file_transfer transfers[2] = {0};
    char output[2][sizeof(payload)] = {0};
    size_t sizes[2] = {0};
    check_not_null(path);
    check_equal(tt_write_file(path, payload, sizeof(payload)), 0);
    check_equal(cflow_io_file_runtime_init(&runtime, &config), SALTS_OK);
    for (size_t i = 0; i < 2u; ++i) {
      check_equal(chttp_file_transfer_open_read(&transfers[i], &runtime, path, sizeof(payload),
                                               sizeof(payload), NULL, NULL), SALTS_OK);
      check_equal(chttp_file_transfer_read(&transfers[i], output[i], sizeof(output[i]), &sizes[i]),
                  CHTTP_FILE_SOURCE_WAIT);
    }
    for (size_t i = 0; i < 2u; ++i) {
      check_equal(chttp_file_test_drive(&runtime, &transfers[i]), SALTS_OK);
      check_equal(chttp_file_transfer_read(&transfers[i], output[i], sizeof(output[i]), &sizes[i]),
                  CHTTP_FILE_SOURCE_DATA);
      check_equal(sizes[i], sizeof(payload));
      check_equal(memcmp(output[i], payload, sizeof(payload)), 0);
      check_equal(chttp_file_transfer_drain_destroy(&transfers[i], &runtime), SALTS_OK);
    }

...
#include "chttp_service_test.http.h"
#include "tinytest.h"
...
FunctionDeclAs(
    fallible, int, &cmeta_type_int, chttp_service_test_add,
    (const AddRequest *, request,
     CMETA_PARAM_IN | CMETA_PARAM_BORROWED, &ADD_REQUEST_PTR_TYPE),
    (AddResponse *, response,
     CMETA_PARAM_OUT | CMETA_PARAM_BORROWED, &ADD_RESPONSE_PTR_TYPE));

int chttp_service_test_add(
    const AddRequest *request, AddResponse *response) {
  if (request == NULL || response == NULL) return -1;
  if (request->left == 13u) return -9;
...
static int chttp_service_test_invoke(
  ...
  bind_status = data_bind_binding_plan_bind_inputs(
      binding, provider, native_options, &frame, diagnostic);
  if (bind_status != DATA_BIND_OK) {
    status = SALTS_EINVAL;
    goto cleanup;
  }

  native_status = chttp_service_test_add(&request, &response);
  bind_status = data_bind_binding_plan_write_outcome(
      binding, provider, &frame, native_status, outcome, diagnostic);
  if (bind_status != DATA_BIND_OK)
    status = SALTS_EPROTO;
...
cleanup:
  ...
  if (data_bind_native_clear(
          native_options, &ADD_REQUEST_DATA, &request,
          sizeof(request), &native_diagnostic) != DATA_BIND_OK &&
      status == SALTS_OK)
    status = SALTS_EPROTO;
  ...
  native_diagnostic =
      (DataBindNativeDiagnostic)DATA_BIND_NATIVE_DIAGNOSTIC_INIT;
  if (data_bind_native_clear(
          native_options, &ADD_RESPONSE_DATA, &response,
          sizeof(response), &native_diagnostic) != DATA_BIND_OK &&
      status == SALTS_OK)
    status = SALTS_EPROTO;
...
spec("CHttp::Service generated MethodPlan runtime") {
  it("mounts generated HTTP mapping without interpreting transport IDL") {
    ...
    check_equal(
        data_bind_create(CHTTP_SERVICE_TEST_SCHEMA, &contract, &bind_error),
        DATA_BIND_OK);
    check_not_null(contract);

    projection = data_bind_http_projection_artifact_find(
        &databind_chttp_service_test_http_projection, "Calc", "Add");
    ...
    check_equal(
        chttp_service_init(&service, &service_config), SALTS_OK);
    check_equal(
    ...
    check_equal(
        chttp_client_init(&client, &client_config), SALTS_OK);
    ...
    check_equal(
        chttp_service_test_call(
            &client, uri, "/add/13?right=4", &response),
        SALTS_OK);

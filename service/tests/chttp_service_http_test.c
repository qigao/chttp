...
static int chttp_service_test_execute(
    void *user,
  ...
  if (plan == NULL || provider == NULL || native_options == NULL ||
      context == NULL || context->http == NULL || result == NULL ||
      diagnostic == NULL || context->response_scratch == NULL ||
      context->response_scratch_bytes == 0u)
    return SALTS_EINVAL;
  ...
  if (bind_status != DATA_BIND_OK) return SALTS_EINVAL;

  chttp_service_test_add(&request, &response);

  body = (char *)context->response_scratch;
  body_size = snprintf(body, context->response_scratch_bytes,
                       "%u", (unsigned int)response.sum);
  if (body_size < 0 ||
      (size_t)body_size >= context->response_scratch_bytes) {
    status = SALTS_EMSGSIZE;
    goto cleanup;
  }

  *result = (chttp_service_result){
      .size = sizeof(*result),
      .status_code = 200u,
...
cleanup:
  ...
  if (data_bind_native_clear(native_options, &ADD_REQUEST_DATA,
                             &request, sizeof(request),
                             &native_diagnostic) != DATA_BIND_OK &&
      status == SALTS_OK)
    status = SALTS_EPROTO;
  native_diagnostic =
      (DataBindNativeDiagnostic)DATA_BIND_NATIVE_DIAGNOSTIC_INIT;
  if (data_bind_native_clear(native_options, &ADD_RESPONSE_DATA,
...
static int chttp_service_test_call(chttp_client *client,
                                   const char *uri,
                                   const char *target,
...
spec("CHttp::Service typed HTTP adapter") {
  it("projects DataBind Service routes into typed native execution") {
    ...
    chttp_service_http_method method =
        CHTTP_SERVICE_HTTP_METHOD_INIT;
    ...
    service_config.contract = contract;
    ...
    method.execute = chttp_service_test_execute;
    check_equal(chttp_service_register_http(
    ...
    check_equal(chttp_service_test_call(
    ...
    check_equal(chttp_client_destroy(
                    &client, CHTTP_SERVICE_TEST_TIMEOUT_MS),
                SALTS_OK);
    check_equal(chttp_server_stop(
                    &server, CHTTP_SERVICE_TEST_TIMEOUT_MS),
                SALTS_OK);
